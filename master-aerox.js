...
async function loadMasterData() {
    ...
    try {
        const { data, error } = await supabase.from('master_aerox').select('*').order('nama');
...
async function handleAddMaster(e) {
    ...
    if (!newName) return;
    try {
        const { error } = await supabase.from('master_aerox').insert([{ nama: newName }]);
        if (error) throw error;
        newNameInput.value = '';
        loadMasterData();
    } catch (error) {
...
async function handleActionClick(e) {
    if (e.target.classList.contains('delete-button')) {
        ...
        if (confirm(`Yakin ingin menghapus "${name}"?`)) {
            try {
                const { error } = await supabase.from('master_aerox').delete().eq('id', id);
                if (error) throw error;
                loadMasterData();
            } catch (error) {
